...
st.markdown("Select your favourite genres and discover great movies!")
...
with tab1:
    ...
    num_recs = st.slider("📋 Number of movies to show:", 5, 20, 10)
    ...
    if st.button("🚀 Find Movies", use_container_width=True):
        if not selected_genres:
            ...
            with st.spinner("Finding movies for you..."):
                ...
                # Sort by number of matching genres (best match first)
                filtered = filtered.copy()
                filtered['match_count'] = filtered['genres'].apply(
                    lambda g: sum(1 for genre in selected_genres if genre in g)
                )
                filtered = filtered.sort_values('match_count', ascending=False)

            st.markdown(f"### 🎬 Top {num_recs} movies for: **{', '.join(selected_genres)}**")
            display_movies(filtered.head(num_recs))
